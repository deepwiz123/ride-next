...
const customerEmailTemplate = (booking: BookingData) => `
  ...
  booking.trip.kids
            ...
            ${
              booking.trip.hourly
                ? `<tr>
                    ...
                    <td style="color: #555;">$${
                      booking.trip.hourlyRate?.toFixed(2) || "N/A"
                    }</td>
                  </tr>
                  <tr>
            ...
            <tr>
              ...
              <td style="color: #555;">$${booking.car.rate.toFixed(2)}</td>
            </tr>
            ${
              booking.fare
                ? `<tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customer, trip, car, bookingId, fare } = body;

    // Email messages
    const emailMsg = {
      // from: process.env.EMAIL_FROM || "no-reply@ridebooking.com",
      from: `Book Ride <${
